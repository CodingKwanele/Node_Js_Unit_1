...
const createUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    const user = await User.register(userDoc, password); // passport-local-mongoose
...
const authenticate = (req, res, next) => {
  passport.authenticate("local", (err, user, info) => {
    ...
    req.logIn(user, (loginErr) => {
      ...
      const redirectTo = req.session?.returnTo || "/dashboard"; // default after login
...
const updateUser = asyncHandler(async (req, res, next) => {
  ...
  if (newPassword) {
    ...
    await userDoc.setPassword(newPassword); // from passport-local-mongoose
  }
